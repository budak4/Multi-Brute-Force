...
def masuk():
    ...
    if "mbasic_logout_button" in str(ismi):
        if "Apa yang Anda pikirkan sekarang" in str(ismi):
            ...
            try:
                requests.get(mbasic.format(parser(ismi, "html.parser").find("a", string="Bahasa Indonesia")["href"]),
                             cookies=cek)
            except:
                pass
        try:
            ...
            ikuti = \
            parser(requests.get(mbasic.format("/zettamus.zettamus.3"), cookies=cek).content, "html.parser").find("a",
                                                                                                                 string="Ikuti")[
                "href"]
